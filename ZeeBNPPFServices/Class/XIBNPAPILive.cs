...
{
    public class XIBNPAPILive : XIDefinitionBase
    {
        //CALLING 
        IntegratedGatewayHost client;
        public Error[] ErrorList { get; set; }
        public string sShopCode { get; set; }
        public XIBNPAPILive() // Load headers,UserCredentials Constructor
        {
            if (client == null)
            {
                ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;
                client = new ZeeBNPPFServices.BNPServiceLive.IntegratedGatewayHost();
            }
        }
        #region XIBNPConfig
