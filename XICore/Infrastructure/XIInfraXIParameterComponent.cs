...
namespace XICore
{
    public class XIInfraXIParameterComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                XIParameter oFDef = new XIParameter();
                XIDXI oXD = new XIDXI();
                oXD.FKiApplicationID = iApplicationID;
                var oXIP = oXD.Get_XIParameterDefinition(ID.ToString());
                if (oXIP.bOK && oXIP.oResult != null)
                {
                    oFDef = (XIParameter)oXIP.oResult;
                }
