...
namespace XICore
{
    public class XIInfraQSStepConfigComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                if (WrapperParms != null && WrapperParms.Count() > 0)
                {
                    ...
                    {
                        StepID = Convert.ToInt32(XIPi1ClickID);
                    }
                    else
                    {
                        StepID = 0;
                ...
                {
                    string sInstanceID = oParams.Where(m => m.sName == "QSStepID").Select(m => m.sValue).FirstOrDefault();
                    ...
                    {
                        if (long.TryParse(sInstanceID, out iStepID))
                        {
                ...
                oQSStep.ID = StepID;
                var oQSStepDef = oQSStep.Get_QSStepConfigDef();
                if (oQSStepDef.bOK && oQSStepDef.oResult != null)
                {
                    oQSStep = (XIDQSStep)oQSStepDef.oResult;
                }
