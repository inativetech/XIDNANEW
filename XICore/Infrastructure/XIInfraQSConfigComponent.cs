...
namespace XICore
{
    public class XIInfraQSConfigComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                {
                    var XIPiQSID = WrapperParms.Where(m => m.sName == "{-iInstanceID}").Select(m => m.sValue).FirstOrDefault(); // oXIAPI.Get_ParamVal(sSessionID, sGUID, null, Prm); //oParams.Where(m => m.sName.ToLower() == Prm.ToLower()).Select(m => m.sValue).FirstOrDefault();
                    if (XIPiQSID != null)
                    {
                        iQSID = Convert.ToInt32(XIPiQSID);
                    }
                    else
                    {
                        iQSID = 0;
                ...
                {
                    string sInstanceID = oParams.Where(m => m.sName == "QSID").Select(m => m.sValue).FirstOrDefault();
                    ...
                    {
                        if (long.TryParse(sInstanceID, out QSID))
                        {
                ...
                XIDQS oQS = new XIDQS();
                oQS.ID = iQSID;
                oQS.FKiApplicationID = iApplicationID;
                var oQSDef = oQS.Get_QSConfigDef();
                if (oQSDef.bOK && oQSDef.oResult != null)
                {
                    oQS = (XIDQS)oQSDef.oResult;
                }
                oQS.ddlApplications = oConfig.Get_AllApplications();
