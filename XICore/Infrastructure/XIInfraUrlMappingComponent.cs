...
namespace XICore
{
    public class XIInfraUrlMappingComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                oUrl.ID = Convert.ToInt32(ID);
                oUrl.FKiApplicationID = iApplicationID;
                var oUrlDef = oUrl.Get_URLDefinition();
                if (oUrlDef.bOK && oUrlDef.oResult != null)
                {
                    oUrl = (XIURLMappings)oUrlDef.oResult;
                }
                oUrl.ddlApplications = oConfig.Get_AllApplications();
