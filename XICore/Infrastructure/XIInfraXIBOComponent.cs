...
namespace XICore
{
    public class XIInfraXIBOComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                XIDBO oFDef = new XIDBO();
                XIDXI oXD = new XIDXI();
                XIDAttribute oXID = new XIDAttribute();
                if (ID == 0)
                {
                    oXD.FKiApplicationID = iApplicationID;
                    oFDef.FKiApplicationID = iApplicationID;
                }
                var oComDef = oXD.Get_BODefinitionALL("", ID.ToString());
                string sDatabase = "";
                if(ID > 0)
                {
                    oFDef = oXD.CopyBOByID(Convert.ToInt32(ID), sDatabase);
                }
                var ddlBOFieldAttributes = oXD.Get_ddlBOFieldAttributes(ID.ToString());
                var ddlBODataSources = oXD.Get_ddlBODataSourceDefinition(ID.ToString());                
                if (oComDef.bOK && oComDef.oResult != null)
                {
                    oFDef = (XIDBO)oComDef.oResult;
                }
                if (ddlBOFieldAttributes.Count() > 0)
                {
                    oFDef.ddlBOFieldAttributes = ddlBOFieldAttributes;
                }
                if(ddlBODataSources.Count() > 0)
                {
                    oFDef.DataSources = ddlBODataSources;
                }
