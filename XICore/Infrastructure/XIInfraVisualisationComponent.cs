...
namespace XICore
{
    public class XIInfraVisualisationComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                if (WrapperParms != null && WrapperParms.Count() > 0)
                {
                    ...
                    {
                        VisualID = Convert.ToInt32(XIPi1ClickID);
                    }
                    else
                    {
                        VisualID = 0;
                ...
                else
                {
                    ...
                    {
                        if (long.TryParse(sInstanceID, out iVisualID))
                        {
                ...
                oVisual.FKiApplicationID = iApplicationID;
                var oQSSecDef = oVisual.Get_VisualisationDef(VisualID);
                if (oQSSecDef.bOK && oQSSecDef.oResult != null)
                {
                    oVisual = (XIVisualisation)oQSSecDef.oResult;
                }
                oVisual.ddlApplications = oConfig.Get_AllApplications();
                oCResult.oResult = oVisual;
