...
namespace XICore
{
    public class XIInfraXIBOAttributeComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                {
                    iBOID = WrapperParms.Where(m => m.sName == "iBOIID").Select(m => m.sValue).FirstOrDefault();
                ...
                oADef.ID = Convert.ToInt32(ID);
                var oAttrDef = oADef.Get_BOAttributeDefinition();
                if (oAttrDef.bOK && oAttrDef.oResult != null)
                {
                    oADef = (XIDAttribute)oAttrDef.oResult;
                }
                oADef.ddlBODataTypes = oADef.Get_BODataTypes();
                oADef.ddlAllBOs = oConfig.Get_AllBOs();
                if (iBOID != "0")
                {
                    oADef.BOID = oADef.BOID;
                    if(oADef.BOID == 0)
                    {
                        oADef.BOID = Convert.ToInt32(iBOID);
                    }
                }
                oCResult.oResult = oADef;
