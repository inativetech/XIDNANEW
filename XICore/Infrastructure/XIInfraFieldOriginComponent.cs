...
namespace XICore
{
    public class XIInfraFieldOriginComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                if (WrapperParms != null && WrapperParms.Count() > 0)
                {
                    ...
                    if (XIPiFieldID != null)
                    {
                        FieldID = Convert.ToInt32(XIPiFieldID);
                    }
                    else
                    {
                        FieldID = 0;
                ...
                {
                    string sInstanceID = oParams.Where(m => m.sName == "FieldID").Select(m => m.sValue).FirstOrDefault();
                    ...
                    {
                        if (long.TryParse(sInstanceID, out FieldID))
                        {
                ...
                var oFieldDef = oFDef.GetFieldDefinition(FieldID.ToString());
                if (oFieldDef.bOK && oFieldDef.oResult != null)
                {
                    oFDef = (XIDFieldOrigin)oFieldDef.oResult;
                }
