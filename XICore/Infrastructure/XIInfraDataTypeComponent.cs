...
namespace XICore
{
    public class XIInfraDataTypeComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                XIDDataType oFDef = new XIDDataType();
                XIDXI oXD = new XIDXI();
                oXD.FKiApplicationID = iApplicationID;
                var oDTDef = oXD.Get_DataTypeDefinition(ID.ToString());
                if (oDTDef.bOK && oDTDef.oResult != null)
                {
                    oFDef = (XIDDataType)oDTDef.oResult;
                }
