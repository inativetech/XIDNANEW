...
namespace XICore
{
    public class XIInfraQueryManagementComponent
    {
        ...
        XIInfraRoles oRoles = new XIInfraRoles();
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                if (WrapperParms != null && WrapperParms.Count() > 0)
                {
                    ...
                    {
                        OneClickID = Convert.ToInt32(XIPi1ClickID);
                    }
                    else
                    {
                        OneClickID = 0;
                ...
                {
                    string sInstanceID = oParams.Where(m => m.sName == "1ClickID").Select(m => m.sValue).FirstOrDefault();
                    ...
                    {
                        if (long.TryParse(sInstanceID, out OneClickID))
                        {
                ...
                var iOrgID = Convert.ToInt32(oParams.Where(m => m.sName == "iOrgID").Select(m => m.sValue).FirstOrDefault());
                ...
                o1Def.ID = Convert.ToInt32(OneClickID);
                o1Def.OrganizationID = iOrgID;
                var oQueryDef = o1Def.Get_QueryDetails();
                if (oQueryDef.bOK && oQueryDef.oResult != null)
                {
                    o1Def = (XID1Click)oQueryDef.oResult;
                    var oRolesTree = oRoles.Get_RolesTree(sCoreDatabase, o1Def.OrganizationID);
                    if(oRolesTree.bOK && oRolesTree.oResult != null)
                    {
                        o1Def.ddlRoles = new List<XIDropDown>();
                    }
                    //ViewBag.Group = (List<XIInfraRoles>)oRolesTree.oResult;
                }
