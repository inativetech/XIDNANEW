...
namespace XICore
{
    public class XIInfraXILinkComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                {
                    var XIPXILinkID = WrapperParms.Where(m => m.sName == "{-iInstanceID}").Select(m => m.sValue).FirstOrDefault(); // oXIAPI.Get_ParamVal(sSessionID, sGUID, null, Prm); //oParams.Where(m => m.sName.ToLower() == Prm.ToLower()).Select(m => m.sValue).FirstOrDefault();
                    if (XIPXILinkID != null)
                    {
                        iXilinkID = Convert.ToInt32(XIPXILinkID);
                    }
                    else
                    {
                        iXilinkID = 0;
                    }
                    sBOName = WrapperParms.Where(m => m.sName == "{XIP|ActiveBO}").Select(m => m.sValue).FirstOrDefault(); // oXIAPI.Get_ParamVal(sSessionID, sGUID, null, Prm); //oParams.Where(m => m.sName.ToLower() == Prm.ToLower()).Select(m => m.sValue).FirstOrDefault();
                    if (sBOName != null)
                    {
                        ActiveBO = sBOName;
                    }
                ...
                {
                    string sInstanceID = oParams.Where(m => m.sName == "XiLinkID").Select(m => m.sValue).FirstOrDefault();
                    ...
                    {
                        if (long.TryParse(sInstanceID, out iXilinkID))
                        {

                        }
                    }
                }
                XILink cXILink = new XILink();
                XIDBO oBOD = new XIDBO();
                oBOD = (XIDBO)oCache.GetObjectFromCache(XIConstant.CacheBO_All, null, ActiveBO);
                cXILink.BOD = oBOD;
                ...
                oCResult.oTraceStack.Add(new CNV { sName = "Stage", sValue = "Params loaded successfully for XiLink Component" });
                cXILink.XiLinkID = Convert.ToInt32(iXilinkID);
                cXILink.iUserID = iUserID;
                cXILink.sOrgName = sOrgName;
                if (cXILink.XiLinkID == 0)
                {
                    cXILink.FKiApplicationID = iApplicationID;
                }
                oXILink = cXILink.Get_XILinkDetails();
                oXILink.ddlApplications = oConfig.Get_AllApplications();
